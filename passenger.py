...
class Passengers:
    ...
    def probability_of_surviving(self, input_age, input_class, input_gender):
        count_surviving_age = self.df.PassengerId.where(self.df.Age == input_age).where(self.df.Survived == 1).count()
        count_surviving_class = self.df.PassengerId.where(self.df.Pclass == input_class).where(self.df.Survived == 1).count()
        count_surviving_gender = self.df.PassengerId.where(self.df.Sex == input_gender).where(self.df.Survived == 1).count()
        ...
        if count_all_age == 0 or count_all_class == 0 or count_surviving_gender == 0:
            return 0
        result = (count_surviving_age/count_all_age +
