...
class Passengers:
    ...
    def sum_male(self):
        return self.df['Sex'].where(self.df['Sex'] == 'male').count()

    def sum_female(self):
        return self.df['Sex'].where(self.df['Sex'] == 'female').count()
    ...
    def most_popular_female_name(self):
        ...
        for i in all_name:
            if str(i) != 'nan':
                ...
                if name_split[0] == 'Miss.' or name_split[0] == 'Mlle.' \
                        or name_split[0] == 'Dr.' or name_split[0] == 'Mme.' \
                        or name_split[0] == 'Ms.':
    ...
    def probability_of_surviving(self, input_age, input_class, input_gender):
        count_survaving_age = self.df['PassengerId'].where(self.df['Age'] == input_age).where(
            self.df['Survived'] == 1).count()
        count_survaving_class = self.df['PassengerId'].where(self.df['Pclass'] == input_class).where(
            self.df['Survived'] == 1).count()
        count_survaving_gender = self.df['PassengerId'].where(self.df['Sex'] == input_gender).where(
            self.df['Survived'] == 1).count()
